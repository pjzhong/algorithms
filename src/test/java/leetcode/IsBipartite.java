...
public class IsBipartite {
    ...
    /**
     * for more info, see the link below and search bipartite
    ...
    public boolean isBipartite(int[][] graph) {
        ...
        for(int i = 0; i < graph.length; i++) {
            if(!marked[i]) {
                isBipartite(graph, i, marked, colored);
            }
        }
        return isBipartite;
    }

    public void isBipartite(int[][] graph, int v, boolean[] marked, boolean[] colored) {
        marked[v] = true;
        for(int i : graph[v]) {
