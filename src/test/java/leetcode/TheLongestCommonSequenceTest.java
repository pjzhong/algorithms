...
public class TheLongestCommonSequenceTest {
    ...
    @Test
    public void test() {
        String[] aCases = {"computer", "didacticA", "tsinghua", "asdfffff", "program", "asdf", "JEQTKVWQLINVKZOC"};
        String[] bCases = {"cute", "advantA", "sina", "asdffff", "algorithm", "ghjk", "EOXXBIIMOHBOXHAR"};
    ...
    private int longestCommonRecursive(String aStr, String bStr, int aIdx, int bIdx, int[][] dp) {
        if(aIdx >= aStr.length() || bIdx >= bStr.length()) { return 0; }

        if(dp[aIdx + 1][bIdx + 1] != 0) { return dp[aIdx + 1][bIdx + 1];}
