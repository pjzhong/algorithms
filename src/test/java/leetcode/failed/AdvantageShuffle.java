...
public class AdvantageShuffle {
     ...
     * Excellent solution
     *
     * @link https://leetcode.com/problems/advantage-shuffle/discuss/149822/JAVA-Greedy-6-lines-with-Explanation
     * */
        ...
        for(int i = 0; i < B.length; i++) {
            ...
            if(entry != null) {
                Map.Entry<Integer, Integer> remove  = null;
                 if(entry.getKey() == B[i]) {
                     remove = entry; map.remove(entry.getKey());
                     entry = map.ceilingEntry(B[i]);
                }
                if(entry != null && B[i] < entry.getKey()) {
                    result[i] = entry.getKey();
                    if(entry.getValue() - 1 <= 0) { map.remove(entry.getKey()); }
