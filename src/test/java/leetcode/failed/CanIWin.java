...
public class CanIWin {
    ...
    /**
     * each player play optimally, so that meas you don't treat they as two separate players, you just have to write
     * a function to play each step optimally. And invoke this function repeatedly as the two plays playing.
     * */
    public boolean helper(int desiredTotal) {
        if(desiredTotal <= 0) { return false; }//The opponent(player1 or player2) reach or excesses the desiredTotal;
        ...
        if(!map.containsKey(key)) {
            for(int i = 1; i < used.length; i++) {
                if(!used[i]) {
                    ...
                    if(!helper(desiredTotal - i)) {//That is the second player
                        map.put(key, true);
                        used[i] = false;
                        return true;
                    }
