package com.pjzhong.leetcode.contest86;
 ...
 *
 * Given an N x N grid of integers, how many 3 x 3 "magic square" subgrids are there?  (Each subgrid
 * is contiguous).
 *
 *
 *
 * Example 1:
 *
 * Input: [[4,3,8,4], [9,5,1,9], [2,7,6,2]] Output: 1 Explanation: The following subgrid is a 3 x 3
 * magic square: 438 951 276
 *
...
public class MagicSquaresInGrid {
  ...
  private boolean isMagic(int[][] grid, int r, int c) {
    for (int i = 0; i <= 2; i++) {
      for (int j = 0; j <= 2; j++) {
        if (10 <= grid[r + i][c + j] || grid[r + i][c + j] <= 0) {
          return false;
        }
      }
    }
    ...
    return (grid[r][c] + grid[r + 1][c + 1] + grid[r + 2][c + 2] == sum) && (
