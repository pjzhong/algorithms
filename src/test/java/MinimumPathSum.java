...
public class MinimumPathSum {
     ...
     * The path to reach (i, j) must be through one the 3 cells:(i - 1, j), (i, j - 1) or (i - 1, j - 1)
     ...
     * Try to implement a recursive solution without memorize the solutions of Subproblems and draw a graph from it
     * to see overlapping Subproblems
     * */
    private int minPathSum(int[][] grid) {
        final int rowSize = grid.length, colSize = grid[0].length;
        for(int i = 1; i < colSize; ++i) { grid[0][i] += grid[0][i - 1]; }
