...
/**
 * Simple echo-back server which listens for incoming stream  connections and echos back whatever it
 *  reads. A Single Selector object is used to listen to the server socket (to accept new
...
public class SimpleEchoClient {
  ...
  protected SelectionKey registerChannel(Selector selector, SocketChannel channel)
      throws Exception {
    if (channel != null) {
  ...
  private static class Handler {
    ...
    private ByteBuffer separateLine = ByteBuffer
        .wrap("\r\n-----------------------------------\r\n".getBytes());
    ...
    /**
     * Sample data handler method for a channel with data ready to read. key A SelectionKey object
     * associated with a channel determined by the selector be be read for reading. If the channel
    ...
    public void sayHello() throws Exception {
      socketChannel.write(prefix);
      String hi = String
          .format("Hi there! I would repeat after you each  %d characters", buffer.capacity());
