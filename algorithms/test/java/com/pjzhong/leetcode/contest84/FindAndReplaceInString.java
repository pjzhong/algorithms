...
/**
 * Find And Replace in String User Accepted: 15 User Tried: 29 Total Accepted: 15 Total Submissions:
 * 33 Difficulty: Medium To some string S, we will perform some replacement operations that replace
 ...
 * "ffff".
 *
 * Using another example on S = "abcd", if we have both the replacement operation i = 0, x = "ab", y
 * = "eee", as well as another replacement operation i = 2, x = "ec", y = "ffff", this second
 * operation does nothing because in the original string S[2] = 'c', which doesn't match x[0] =
 * 'e'.
 *
 ...
 * "eeebffff" Explanation: "a" starts at index 0 in S, so it's replaced by "eee". "ed" starts at
 * index 2 in S, so it's replaced by "ffff". Example 2:
 *
 * Input: S = "abcd", indexes = [0,2], sources = ["ab","ec"], targets = ["eee","ffff"] Output:
 * "eeecd" Explanation: "a" starts at index 0 in S, so it's replaced by "eee". "ec" doesn't starts
 * at index 2 in the original S, so we do nothing. Notes:
 *
 * 0 <= indexes.length = sources.length = targets.length <= 100 0 < indexes[i] < S.length <= 1000
...
public class FindAndReplaceInString {
  ...
  public void test() {
    ...
    String[][] soruce = {
        ...
        {"bje", "yi", "wm", "uv", "tk", "bl", "tpm", "orrz", "go", "ur", "bp", "giy", "gh", "ooie",
            "jcm", "ko"},
        {"a", "cd"},
        {"ab", "ec"},
        {"a", "cd"}};
    String[][] target = {
        ...
        {"fnb", "r", "j", "eqq", "t", "b", "bra", "fhbdm", "un", "iw", "wfr", "okz", "p", "bgk",
            "tu", "d"},
        {"eee", "ffff"},
        {"e", "ffff"},
        {"", "f"}};
