...
/**
 * If you want to visualization the insert and remove process of this tree see this
 * link:https://www.cs.usfca.edu/~galles/visualization/RedBlack.html
 *
 * RedBlockTree only one rotates and multi recolored after delete, than balance property will be
 * hold again. AVLBinarySearchTree need O(logN) times
 */
public class RedBlackTree<Key extends Comparable<Key>, Value> extends BinaryTree<Key, Value> {
  ...
  /**
   * for more info, see the links blow https://www.geeksforgeeks.org/red-black-tree-set-3-delete-2/
   * http://cs.lmu.edu/~ray/notes/redblacktrees/ http://www.xuetangx.com/courses/course-v1:TsinghuaX+30240184_2X+sp/pdfbook/0/
   *
  ...
  private RedBlackNode grandParentOf(RedBlackNode node) {
    return (hasParent(node) && hasParent(node.getParent())) ? (RedBlackNode) node.getParent()
        .getParent() : null;
  }
